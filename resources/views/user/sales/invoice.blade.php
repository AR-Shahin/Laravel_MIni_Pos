                    ...
                    <table class="table table-light table-bordered">
                        ...
                        @foreach($invoice->items as $key => $item)
                            <tr>
                                ...
                                <td>${{ $item->total }}</td>
                                <td>
                                    <form method="POST" action=" {{ route('user.sales.invoices.delete_item', ['id' => $user->id, 'invoice_id' => $item->id ,'item_id'=>$item->id]) }} " style="display:inline-block">
                                        @csrf
                                        @method('DELETE')
...
<div class="modal fade" id="addProduct" tabindex="-1" role="dialog" aria-labelledby="addProduct" aria-hidden="true">
    <form action="{{ route('user.sales.invoices.add_item',['id' => $user->id, 'invoice_id' => $invoice->id] )  }}" method="POST">
        @csrf
